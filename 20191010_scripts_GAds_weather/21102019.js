...
function buildCampaignRulesMapping(campaignRulesData) {
  ...
  //Logger.log('buildCampaignRulesMapping::__Campaign Mapping: %s', campaignMapping);
...
function buildWeatherConditionMapping(weatherConditionData) {
  ...
  //Logger.log('buildWeatherConditionMapping::__Weather condition mapping: %s', weatherConditionMapping);
...
function buildLocationMapping(geoTargetData) {
  ...
  for (var i = 0; i < geoTargetData.length; i++) {
    ...
    //FORMATTER LA COLONNE LonLat POUR POUVOIR L'INTEGRER DANS UNE URL GET
    var tmp = geoTargetData[i][2]
    tmp = tmp.replace(', ', '&lon=')
    var geoTarget = 'lat='+tmp

    locationDetails.geoCodes.push(geoTargetData[i][1]);
    locationDetails.lonLat.push(geoTarget);
    locationMapping[locationName] = locationDetails;
  }
  //Logger.log('buildWeatherConditionMapping::__Location Mapping: %s', locationMapping);
...
function applyRulesForCampaign(campaignName, campaignRules, locationMapping,
  ...
  //Logger.log(campaignRules.length)
  for (var i = 0; i < campaignRules.length; i++) {
    var bidModifier = 1;
    var campaignRule = campaignRules[i];
    Logger.log("campaignRules: %s", campaignRules)

    // Get the weather for the required location.
    var locationDetails = locationMapping[campaignRule.location];
    //Logger.log(locationDetails.lonLat)
    //Logger.log(campaignRule.location)
    if(locationDetails.lonLat[0] == "lat=")  IS_city = false
    //Logger.log('locationDetails.lonLat: %s', locationDetails.lonLat);
    var weather = getWeather(campaignRule.location);
    var weatherAround = getAroundWeather(campaignRule.location, locationDetails.lonLat[0]);
    //Logger.log('applyRulesForCampaign::weatherAround: %s', weatherAround)
    ...
    //Logger.log('applyRulesForCampaign::weatherConditionRules: %s', weatherConditionRules)

    /*
    evaluateWeatherRules(weatherConditionRules, weather)
    evaluateWeatherRulesAround(weatherConditionRules, weatherAround)
    exit()
    */

    // Evaluate the weather rules.
    var evalWeather = evaluateWeatherRules(weatherConditionRules, weather)
    var evalWeatherAround = {}
    for(a in weatherAround.list){
      evalWeatherAround[weatherAround.list[a].name] = evaluateWeatherRules(weatherConditionRules, weatherAround.list[a])
    }
    Logger.log("campaignRule.location: %s, evalWeather: %s, evalWeatherAround: %s", campaignRule.location, evalWeather, evalWeatherAround)

    /*
    if(!evalWeather)
      Logger.log('applyRulesForCampaign::evalWeather, weather: %s, %s', evalWeather, weather)
    if(!evalWeatherAround)
      Logger.log('applyRulesForCampaign::evalWeatherAround, weatherAround: %s, %s', evalWeatherAround, weatherAround)
    */
    ...
    if (evalWeather) {
      ...
      if (TARGETING == 'LOCATION' || TARGETING == 'ALL') {
        ...
        var lonLat = campaignRule.targetedOnly ?
          locationDetails.lonLat : null;
        adjustBids(campaignName, geoCodes, bidModifier, lonLat);
      }
...
function evaluateWeatherRules(weatherRules, weather) {
  ...
  /*Logger.log("\n\n\n\nweatherRules.temperature, temperature: %s, %s", weatherRules.temperature, temperature)
  Logger.log("weatherRules.precipitation, precipitation: %s, %s", weatherRules.precipitation, precipitation)
  Logger.log("weatherRules.wind, windspeed: %s, %s\n\n\n\n", weatherRules.wind, windspeed)
  */
...
function evaluateWeatherRulesAround(weatherRules, weatherAround) {
  //Logger.log("weatherRules: %s", weatherRules)
  //Logger.log("weatherAround: %s", weatherAround)
  ...
  for(el in list){
    ...
    //Logger.log("tmp.rain: %s, tmpbis[0][1]: %s", tmp.rain, tmpbis[0][1])
    if(tmp.rain != null) tmpbis[1][1]++
    tmpbis[0][2] += tmp.wind != null ? tmp.wind.speed : 0
    if(tmp.wind != null) tmpbis[1][2]++
    //Logger.log("tmp: %s", tmp)
    //Logger.log("tmpbis: %s", tmpbis)
  }
  ...
  /*
  Logger.log("weather: %s", weather)
  Logger.log("weatherRules: %s", weatherRules)
  Logger.log("evaluateWeatherRulesResult: %s", evaluateWeatherRulesResult)
  */
...
function matchesBelow(condition, value) {
  ...
  //Logger.log("matchesBelow|||condition: %s, value: %s", condition, value)
...
function matchesAbove(condition, value) {
  ...
  //Logger.log("matchesAbove|||condition: %s, value: %s", condition, value)
  if (value > conditionParts[1]) {
    return true;
  }
  return false;
...
function matchesRange(condition, value) {
  ...
  //Logger.log("matchesRange|||condition: %s, value: %s", condition, value)
...
function getWeather(location) {
  if (location in WEATHER_LOOKUP_CACHE) {
    //Logger.log('getWeather::__Cache hit...');
  ...
  /*
  Logger.log("getWeather::url: %s", url)
  exit()
  */
  Logger.log("getWeather::url: %s", url)
...
function getAroundWeather(location, coord) {
  if (location in WEATHER_LOOKUP_CACHE_AROUND) {
    //Logger.log('getAroundWeather::__Cache hit...');
  ...
  Logger.log(location)
  Logger.log(coord)
  Logger.log("getAroundWeather::url: %s", url)
  exit()
  */
  Logger.log("getAroundWeather::url: %s", url)
  var response = UrlFetchApp.fetch(url);
...
function adjustBids(campaignName, geoCodes, bidModifier, lonLat) {
  ...
  //Logger.log("adjustBids::campaign: %s", campaign)
  if (!campaign) return null;

  //Logger.log('yoooooooooooooooooooooooooooooooo')
  //Logger.log(AdsApp.campaigns())
  ...
  while (locations.hasNext()) {
    ...
    Logger.log("bool: %s, cpt: %s", bool, cpt)
    Logger.log("campaignName: %s", campaignName)
    Logger.log("geoCodes: %s", geoCodes)
    Logger.log("bidModifier: %s", bidModifier)
    Logger.log("location.getId(): %s", location.getId())
    Logger.log("currentBidModifier: %s, bidModifier: %s", currentBidModifier, bidModifier)
    Logger.log("geoCodes.indexOf(location.getId()): %s, currentBidModifier != bidModifier: %s", geoCodes.indexOf(location.getId()), (currentBidModifier != bidModifier))
    */

    if (bool) {
      Logger.log("adjustBids::__Adding targeting for : %s", geoCodes)
      campaign.addLocation({
        id: geoCodes[0],
        bidModifier: bidModifier
      });

...
var OPEN_WEATHER_MAP_API_KEY = '1d4d40d420b23984f01af033191f2d5a';
...
function buildLocationMapping(geoTargetData) {
  ...
  for (var i = 0; i < geoTargetData.length; i++) {
    ...
    locationDetails.lonLat.push(geoTargetData[i][2]);
...
function applyRulesForCampaign(campaignName, campaignRules, locationMapping,
  ...
  for (var i = 0; i < campaignRules.length; i++) {
    ...
    var weather_ = getWeather(campaignRule.location);
    ...
    if (evaluateWeatherRules(weatherConditionRules, weather) && evaluateWeatherRules_(weatherConditionRules, weather_)) {
...
function validateApiKey() {
  if (OPEN_WEATHER_MAP_API_KEY == '1d4d40d420b23984f01af033191f2d5a') {

...
function main() {
  ...
  Logger.log("--------------------------------")
  Logger.log(campaignMapping)
  Logger.log(weatherConditionMapping)
  Logger.log(locationMapping)
  Logger.log("--------------------------------")
  // Apply the rules.
  ...
  */
  for (var i = 0; i < campaignRules.length; i++) {
    ...
    */
    ...
    var location = campaignRule.location;
    var locationDetails = locationMapping[campaignRule.condition];
    var weather = getWeather(campaignRule.location);
    Logger.log('Weather for %s: %s', location, weather);

    // Get the weather rules to be checked.
    var weatherConditionName = campaignRule.condition;
    var weatherConditionRules = weatherConditionMapping[campaignRule.location];
    Logger.log("campaignRule.location")
    Logger.log(campaignRule.location)
    Logger.log("weatherConditionName")
    Logger.log(weatherConditionName)
    Logger.log("weatherConditionMapping")
    Logger.log(weatherConditionMapping)
    Logger.log("weatherConditionRules")
    Logger.log(weatherConditionRules)
    /*
    */
...
function evaluateWeatherRules(weatherRules, weather) {
  ...
  if (weather.rain && weather.rain['2h']) {
    precipitation = weather.rain['2h'];
  }
  ...
  /*
  Logger.log("evaluateMatchRules(weatherRules.temperature, temperature): %s", evaluateMatchRules(weatherRules.temperature, temperature))
  Logger.log("evaluateMatchRules(weatherRules.precipitation, precipitation): %s", evaluateMatchRules(weatherRules.precipitation, precipitation))
  Logger.log("evaluateMatchRules(weatherRules.wind, windspeed): %s", evaluateMatchRules(weatherRules.wind, windspeed))
  */

  Logger.log("weatherRules: %s", weatherRules)
  Logger.log("weather: %s", weather)
  Logger.log("weather.rain: %s", weather.rain)
  Logger.log("weather.rain['3h']: %s", weather.rain['3h'])
  Logger.log("weatherRules.temperature: %s", weatherRules.temperature)
  Logger.log("weatherRules.precipitation: %s", weatherRules.precipitation)
  Logger.log("weatherRules.wind: %s", weatherRules.wind)
...
function matchesBelow(condition, value) {
  ...
  conditionParts = condition.split(' ');
    Logger.log("condition matchesBelow")
    Logger.log(condition)
    Logger.log(conditionParts[1])
...
function matchesAbove(condition, value) {
  ...
  if (conditionParts[0] != 'above') {
    ...
    Logger.log("condition matchesAbove")
    Logger.log(condition)
    Logger.log(conditionParts[1])
...
function matchesRange(condition, value) {
  ...
  if (conditionParts[1] != 'to') {
    ...
    Logger.log("condition matchesRange")
    Logger.log(condition)
    Logger.log(conditionParts[1])
...
function getWeather(location) {
  if (location in WEATHER_LOOKUP_CACHE) {
    ...
    Logger.log(WEATHER_LOOKUP_CACHE);
...
function validateAndGetSpreadsheet(spreadsheeturl) {
  if (spreadsheeturl !== 'https://docs.google.com/spreadsheets/d/12Wyu3BaQqUxgQL5-CWzl1QFF5GB54nTTQmyIabkUxWA/edit#gid=0') {
...
function validateApiKey() {
  if (OPEN_WEATHER_MAP_API_KEY !== '4b2c32e40644783f3c0ccd3becb8e27d') {

...
function applyRulesForCampaign(campaignName, campaignRules, locationMapping,
  ...
  for (var i = 0; i < campaignRules.length; i++) {
    ...
    var weatherAround = {}
    if(IS_city)
       weatherAround = getAroundWeather(campaignRule.location, locationDetails.lonLat[0]);
    ...
    */
    ...
    if(IS_city)
       evalWeatherAround = evaluateWeatherRulesAround(weatherConditionRules, weatherAround)
    else {
      evalWeatherAround = true
      Logger.log("Localisation: %s\nevalWeatherAround est directement initalisé à true. evalWeatherAround: %s\nCar %s n'est pas une ville", campaignRule.location, evalWeatherAround, campaignRule.location)
    }
    ...
    if (evalWeather && evalWeatherAround) {
...
function evaluateWeatherRulesAround(weatherRules, weatherAround) {
  ...
  for(el in list){
    boolbis = true
    tmp = list[el]
    tmpbis[0][0] += tmp.main != null ? toCelcius(tmp.main.temp) : 0
    if(tmp.main != null) tmpbis[1][0]++
    tmpbis[0][1] += tmp.rain != null ? tmp.rain['3h'] : 0
    if(tmp.rain != null) tmpbis[1][1]++
    tmpbis[0][2] += tmp.wind != null ? tmp.wind.speed : 0
    if(tmp.wind != null) tmpbis[1][2]++
  }

  tmpbis[0][0] /= tmpbis[1][0]
  tmpbis[0][1] /= tmpbis[1][1]
  tmpbis[0][2] /= tmpbis[1][2]
  var weather = {main: {temp: tmpbis[1][1]}, rain: {'3h': tmpbis[0][1]}, wind: {speed: tmpbis[1][2]}}

  return evaluateWeatherRules(weatherRules, weather)

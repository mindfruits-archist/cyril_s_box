...
function applyRulesForCampaign(campaignName, campaignRules, locationMapping,
  ...
  for (var i = 0; i < campaignRules.length; i++) {
    ...
    var weatherAround
    if(locationDetails.lonLat[0] != "lat=") weatherAround = getAroundWeather(campaignRule.location, locationDetails.lonLat[0]);
    else evalWeatherAround = true
    ...
    Logger.log('applyRulesForCampaign::__Weather for: %s, %s', locationDetails, weather);
    Logger.log('applyRulesForCampaign::__Arounds weather for: %s', weatherAround);
    /*
    ...
    */
    ...
    if(locationDetails.lonLat[0] != "lat=") evalWeatherAround = evaluateWeatherRulesAround(weatherConditionRules, weatherAround)
    else evalWeatherAround = true

    if(!evalWeather)
      Logger.log('applyRulesForCampaign::weather: %s', weather)
...
function evaluateWeatherRulesAround(weatherRules, weatherAround) {
  ...
  var bool = true
  for(el in list){
    //Logger.log("el: %s", list[el])
    if(!evaluateWeatherRules(weatherRules, list[el]))
      bool = false
  }
  Logger.log("evaluateWeatherRulesAround::bool: %s", bool)
  return bool
}
